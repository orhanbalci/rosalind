...
fn main() {
    ...
    dnas.iter().enumerate().inspect(|index_first_dna|{
        dnas.iter().skip(index_first_dna.0).enumerate().inspect(|index2_second_dna|{
...
fn pretty_print<T : fmt::Display>(matrix : &mut[&mut[T]]){
    matrix.iter().inspect(|row|{
        row.iter().inspect(|element|{
            print!("{} ",element);
        }).collect::<Vec<_>>();
        println!("");
    }).collect::<Vec<_>>();
