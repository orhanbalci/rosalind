...
fn main() {
    ...
    for (i, el1) in values.iter().enumerate() {
        for (i2, el2) in values.iter().skip(i + 1).enumerate() {
            ...
            if el1.chars().skip(el1.len() - 3).take(3).collect::<String>() ==
               el2.chars().take(3).collect::<String>() {
                let first_node = keys.iter().skip(i).next().unwrap();
                let second_node = keys.iter().skip(i2 + i + 1).next().unwrap();
                // println!("({} {}, {} {})", i,el1, i2+i+1,el2);
                println!("{} {}",
                         first_node.chars().skip(1).collect::<String>(),
                         second_node.chars().skip(1).collect::<String>());
            }

            if el2.chars().skip(el2.len() - 3).take(3).collect::<String>() ==
               el1.chars().take(3).collect::<String>() {
