...
fn get_mutation_type( base_pair : (base_type,base_type))->point_mutation_type{
    match base_pair{
        (base_type::purine,base_type::purine) | (base_type::pyrimidine,base_type::pyrimidine)  => point_mutation_type::transition,
        (base_type::purine,base_type::pyrimidine) | (base_type::pyrimidine,base_type::purine)  => point_mutation_type::transversion
    }
}
...
fn main(){
    ...
    for i in 0..lines[0].len() {
        if lines[0].chars().nth(i).unwrap() != lines[1].chars().nth(i).unwrap() {
            match get_mutation_type((get_base_type(lines[0].chars().nth(i).unwrap()).unwrap(), get_base_type(lines[1].chars().nth(i).unwrap()).unwrap())) {
