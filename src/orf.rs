...
fn main() {
    let dnas = protein_utilities::read_fasta(".\\src\\orf.txt");
    let rna = protein_utilities::transcibe(dnas[0].clone());
    ...
    let complement_rna =
        protein_utilities::transcibe(protein_utilities::reverse_complement(&dnas[0].clone()));
    ...
    print_vector(&result, &mut result_string);
    protein_utilities::write_string_to_file("results.txt", &result_string);
}
...
fn find_open_reading_frames(rna: String,
                            ...
                            ct: &protein_utilities::CodonTable)
                            -> Vec<String> {
    ...
    for chunk in rna.clone().into_bytes()[skip_char_count..].chunks(3).enumerate() {
        if protein_utilities::is_start_codon(String::from_utf8_lossy(chunk.1).as_ref()) {
    ...
    for index in indexes {
        for chunk in rna.clone().into_bytes()[skip_char_count..].chunks(3).skip(index) {
            if protein_utilities::is_stop_codon(String::from_utf8_lossy(chunk).as_ref()) {
